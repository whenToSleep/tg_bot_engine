...
def load_game_data():
    ...
    try:
        # Load mobs
        mobs = data_loader.load_category(
            str(data_dir / "mobs"),
            "mob",
            str(data_dir / "schemas" / "mob_schema.json")
        )
        logger.info(f"✓ Loaded {len(mobs)} mobs")
        
        # Load items
        items = data_loader.load_category(
            str(data_dir / "items"),
            "item",
            str(data_dir / "schemas" / "item_schema.json")
        )
        logger.info(f"✓ Loaded {len(items)} items")
        
        # Optional: Load quests if directory exists
        quests_dir = data_dir / "quests"
        if quests_dir.exists():
            quests = data_loader.load_category(
                str(quests_dir),
                "quest",
                str(data_dir / "schemas" / "quest_schema.json")
            )
            logger.info(f"✓ Loaded {len(quests)} quests")
        
        # Optional: Load locations if directory exists
        locations_dir = data_dir / "locations"
        if locations_dir.exists():
            locations = data_loader.load_category(
                str(locations_dir),
                "location",
                str(data_dir / "schemas" / "location_schema.json")
            )
...
async def main():
    ...
    logger.info(f"✓ Modules initialized ({event_bus.get_subscriber_count()} event subscribers)")
